...
class SimRunner:
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            if now - last >= 1.0:
                self.real_sps = cnt / (now - last); cnt = 0; last = now
            delay = 1.0 / max(0.5, self.target_sps) - (time.perf_counter() - t0)
            if delay > 0:
                time.sleep(delay)
    ...
    def render_state(self):
        ...
        st["runner"] = {"status": self.status, "target_sps": round(self.target_sps, 1),
...
class Manager:
    ...
    def control(self, sim_id, action, value=None):
        ...
        if action == "pause":
            ...
            r.target_sps = max(0.5, float(value))
