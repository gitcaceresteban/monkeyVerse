...
from .language import N_SYMBOLS
...
class World:
    def __init__(self, cfg, rng: np.random.Generator):
        ...
        self.lang = np.zeros((self.h, self.w, N_SYMBOLS), dtype=np.float32)
        self.meat = np.zeros((self.h, self.w), dtype=np.float32)
        # movement heatmap: where agents have actually been (for the analytics tab)
    ...
    def update(self, tick: int) -> list[str]:
        ...
        self.lang *= 0.55   # utterances are transient
        # carrion rots
        self.meat *= (1.0 - c.corpse_decay)
        self.heatmap *= c.heatmap_decay
    ...
    def render_layers(self, out_w: int, out_h: int) -> dict[str, Any]:
        ...
        heat = self.heatmap[ix]
        hmax = float(heat.max()) or 1.0
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            ...
            "veg": self.veg, "pher": self.pher, "lang": self.lang,
            "meat": self.meat, "materials": self.materials, "heatmap": self.heatmap,
    ...
    def load_state(self, s: dict[str, Any]) -> None:
        ...
        self.lang = np.asarray(s.get("lang", s.get("sound")), dtype=np.float32)
        self.meat = np.asarray(s["meat"], dtype=np.float32)
        self.materials = np.asarray(s["materials"], dtype=np.float32)
        self.heatmap = np.asarray(s.get("heatmap", np.zeros((self.h, self.w))), dtype=np.float32)
